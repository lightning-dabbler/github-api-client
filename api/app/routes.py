...
def home():
    ...
    response = {"status_code": 200, "resources": resources}
...
def search_resource():
    ...
    response = {
        ...
        "endpoint": {"values": ["repositories", "users", "commits"], "optional": False},
        "query": {
...
def query_search(endpoint, query):
    ...
    if type(sort) == str:
        sort = helpers.str_url_params_fix(sort)
    if type(order) == str:
        order = helpers.str_url_params_fix(order)
...
def query_emojis():
    ...
    if emoji:
        emoji = helpers.str_url_params_fix(emoji)
